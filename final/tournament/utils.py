    ...
    def __call__(self, team1_state, team2_state, soccer_state, actions, team1_images=None, team2_images=None,
        ...
        if team1_images and team2_images :
            self._writer.append_data(np.array(video_grid(team1_images, team2_images,
                                                         'Blue: %d accel %s steer %s brake %s' % (soccer_state['score'][1], actions[1].get('acceleration'),actions[1].get('steer'),actions[0].get('brake')),
                                                         'Red: %d accel %s steer %s brake %s' % (soccer_state['score'][0],actions[0].get('acceleration'),actions[0].get('steer'),actions[0].get('brake')))))
        else:
