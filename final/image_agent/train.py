    ...
    for epoch in range(args.num_epoch):
        ...
        for data in train_data:
            ...
            if train_logger is not None:
                ...
                if global_step % 20 == 0:
                    log(train_logger, img, loc, pred_loc, puck, pred_puck, global_step)
        ...
        for data in valid_data:
            ...
            loc = data[2].to(device)
            pred_puck, pred_loc = model(img)
            # force puck prediction between 0 and 1 on validation
...
def log(logger, img, loc, pred_loc, puck, pred_puck, global_step):
    import matplotlib.pyplot as plt
    import torchvision.transforms.functional as TF
    fig, ax = plt.subplots(1, 1)
    ax.imshow(TF.to_pil_image(img[0].cpu()))
    WH2 = np.array([img.size(-1), img.size(-2)])/2
    ax.annotate('Puck %s' % puck[0].cpu().detach().numpy(),(25.,25.),color='green')
    ax.annotate('Predicted %s' % pred_puck[0].cpu().detach().numpy(), (25.,50),color='red')
