...
def seek_point(location,direction,destination,forward=None):
    ...
    if new_vector_magnitude < 4.0: 
        ...
        return np.random.rand(), np.random.rand(), bool(random.getrandbits(1))
...
class Team:
    ...
    def new_match(self, team: int, num_players: int) -> list:
        ...
        self.p0_corner_point[1] = self.p1_box_centerpoint[1] - 5.0*np.sign(self.p1_box_centerpoint[1])
    ...
    def check_offensive_box(self, current_location,current_direction):
        ...
        if np.sign(current_location[1]) == np.sign(self.target_goal[1]) and np.abs(current_location[1]) > 54:
            print('in offensive box, x-dir',current_direction[0])
            if np.abs(current_direction[0]) > 0.8:
    ...
    def act(self, player_state, player_image):
        ...
        p1_puck_onscreen_threshold = 4.5
        ...
        if (p0_puck_onscreen<p0_puck_onscreen_threshold or p0_puck_dist < -0.2) and self.global_step>0:
            ...
            if p0_puck_dist < .225 and p0_puck_dist >= -0.1 and np.abs(p0_puck_loc[0]) <0.5:
                ...
                if np.sign(p0_goal_direction[1]) == np.sign(p0_kart_direction[1]) and np.abs(p0_goal_direction[0] - p0_kart_direction[0]) < 0.5:
                    ...
                    if np.abs(p0_goal_alignment_value) > 0.01:
                        print('p0 kart and goal x-value are not aligned yet')
                        ...
                        if p0_offensive_box != 0.0:
                            ...
                            p0_steer_addition = 1.5* p0_steer_addition
                        elif p0_on_x_edge or p0_on_y_edge or p0_in_corner:
                            p0_steer_addition = 2.5 * p0_puck_wall_steer
                            print('p0 I think puck near wall, add higher steer',p0_steer_addition)
                        else: 
                            p0_steer_addition = 1.5 * p0_goal_alignment_value 
                ...
                if np.abs(p0_puck_loc[0]) > 0.05 and p0_puck_dist <= 0.2 and p0_puck_loc[1] > 0.1:
                    p0_accel = 0
                    p0_steer_addition = 0.0
                    p0_brake = True
                    p0_steer = -.5 *np.sign(p0_puck_loc[0])
        ...
        if self.global_step > 15 and ((np.abs(self.p0_prev_locations[self.global_step-1][0] - p0_current_location[0]) < 0.02 and np.abs(self.p0_prev_locations[self.global_step-1][1] - p0_current_location[1]) < 0.02) and (np.abs(self.p0_prev_vel_magnitudes[self.global_step-1]) < 0.5 and np.abs(p0_current_vel_magnitude)   < 0.5)) or np.abs(p0_kart_front[1]) > 64.0:
            ...
            if np.sign(p0_kart_front[1]) == np.sign(p0_kart_direction[1] or np.sign(p0_kart_front[0]) == np.sign(p0_kart_direction[0])):
                print('p0 kart facing direction of location, back up')
                p0_accel = 0
                p0_brake = True
                p0_steer = np.sign(p0_current_location[0])
            else:
                print('p0 kart facing opposite of location, go forward')
                p0_accel = 1
                p0_brake = False
                p0_steer = -np.sign(p0_current_location[0])
            '''
        ...
        if p1_puck_onscreen < p1_puck_onscreen_threshold or p1_puck_dist > 0.35:
            # print('p1 puck not onscreen or far away')
            if not p1_in_box:
                p1_steer, p1_accel, p1_brake = seek_point(p1_kart_front.numpy(),p1_kart_direction,self.p1_box_centerpoint,forward=False)
                '''
                if np.abs(p1_kart_front[1]) < 64: 
                    p1_accel = 0.0
                    p1_brake = True
                    p1_steer = -np.sign(p1_kart_front[1])*p1_center_vector[0]
                else:
                    p1_accel = 0.25
                    p1_brake = False
                    p1_steer = np.sign(p1_kart_front[1])*p1_center_vector[0]
